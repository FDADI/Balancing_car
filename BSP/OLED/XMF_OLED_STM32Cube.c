...
//-----显示汉字
void OLED_ShowChinese(unsigned char x,unsigned char y,unsigned char *z)			 
{      			    
	unsigned char t,i;
	while(*z != '\0')
	{
		for(t=0;t<20;t++)
		{
			...
			{
				OLED_Set_Pos(x,y);		for(i=0;i<16;i++){OLED_WR_Byte(Hzk[t].hzk_32[i],OLED_DATA);}
				OLED_Set_Pos(x,y+1);	for(i=0;i<16;i++){OLED_WR_Byte(Hzk[t].hzk_32[i+16],OLED_DATA);}							
			}
		}
		x+=16;
		if(x>120){x=0;y+=2;}
...
{	unsigned char t,i;
	...
	{	if(*chr>='!'&&*chr<='~')
		...
		for(t=0;t<20;t++)
		{
			if(Hzk[t].hzk_2[0]==*chr && Hzk[t].hzk_2[1]==*(chr+1))
			{
				OLED_Set_Pos(x,y);		for(i=0;i<16;i++){OLED_WR_Byte(Hzk[t].hzk_32[i],OLED_DATA);}
				OLED_Set_Pos(x,y+1);	for(i=0;i<16;i++){OLED_WR_Byte(Hzk[t].hzk_32[i+16],OLED_DATA);}							
			}
